  ...
  elements.forEach(function (element) {
    var elementConfig =
      typeof element.target !== 'undefined'
        ? parseConfig(element.target)
        : parseConfig(element);

    if (elementConfig.duration <= 0) {
      return (element.innerHTML = elementConfig.end.toFixed(
        elementConfig.decimals
      ));
    }
    if (
...
function parseConfig(element) {
  ...
  for (var i = 0; i < configValues.length; i++) {
    ...
    newConfig[valueInd.toLowerCase()] =
      valueInd.toLowerCase() == 'duration'
        ? parseInt(castDataType(configValues[i].value) * 1000)
        : castDataType(configValues[i].value);
  }
